...
step (position aim length depth) (forward x) = position aim (x + length) (aim * x + depth)
step (position aim length depth) (down x) = position (x + aim) length depth
step (position zero length depth) (up x) = position zero length depth
step (position (suc aim) length depth) (up zero) = position (suc aim) length depth
step (position (suc aim) length depth) (up (suc x)) = step (position aim length depth) (up x)
...
prog0 = (forward 5)
      ...
      ∷ (down 8)
      ∷ ( forward 2 ) ∷ []
test0 : exec prog0 origin ≡ position 10 15 60
test0 = refl
...
showResult (just (position a l d)) = putStrLn $ show (l * d)
showResult nothing = putStrLn "error"

main : Main
main = run $
  readFiniteFile "input-p2"
